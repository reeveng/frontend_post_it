...
export class RegisterComponent implements OnInit {
      ...
      passwordGroup: this.fb.group(
        {
          password: ['', [
            ...
            Validators.pattern("^((?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])|(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[^a-zA-Z0-9])|(?=.*?[A-Z])(?=.*?[0-9])(?=.*?[^a-zA-Z0-9])|(?=.*?[a-z])(?=.*?[0-9])(?=.*?[^a-zA-Z0-9])).{8,}$")
          ]],
  ...
  getErrorMessage(errors: any) {
    ...
    if (errors.required) {
      ...
      return "Passwords must contain the following: upper case (A-Z), lower case (a-z), number (0-9) and special character (e.g. !@#$%^&*)";
    }
  }
